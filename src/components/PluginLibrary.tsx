...
export function PluginLibrary({ registry = false }: { registry?: boolean }) {
  ...
  return <main id="main"><div className="container">
    ...
    {!registry && <section className="featured-world" aria-labelledby="featured-title">
      ...
      <div className="featured-copy"><p className="eyebrow"><span className="line-marker" /> FEATURED RELEASE <span className="release-label">v{featured.versions[0].version}</span></p><h2 id="featured-title">Voidscape</h2><p className="featured-tagline">A world beneath your world.</p><p className="featured-description">เปิดมิติใหม่ใต้ Bedrock สำรวจเกาะลอยฟ้า<br />เผชิญหน้ากับบอส และค้นพบไอเทมพิเศษ</p><Link href="/plugins/voidscape" className="button button-light">Explore Voidscape <ArrowUpRight size={17} /></Link></div>
